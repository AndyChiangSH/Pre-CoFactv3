...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        id, claim_id, claim, evidence, questions, claim_answers, evidence_answers = self.data[idx]["id"], self.data[idx]["claim_id"], self.data[idx]["claim"], self.data[idx]["evidence"], self.data[idx]["question"], self.data[idx]["claim_answer"], self.data[idx]["evidence_answer"]
        if config["use_features"]:
        ...
        if config["use_features"]:
            return (claim, evidence, claim_qas, evidence_qas, feature)
        else:
            return (claim, evidence, claim_qas, evidence_qas)
...
if __name__ == '__main__':
    ...
    model_path = f"generate_label/fakenet/model/{input_argument['model']}"
    ...
    with torch.no_grad():
        ...
        for loader_idx, item in tqdm(enumerate(dataloader), total=len(dataloader), desc='Step: '):
            ...
            claim_texts, evidence_texts, claim_qas, evidence_qas = item[0], item[1], item[2], item[3]
            if config["use_features"]:
                features = item[4].to(device)
    ...
    output_folder_path = f"generate_label/fakenet/label/{input_argument['model']}"
